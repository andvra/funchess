...
def print_board(board, filename, no_plys):
    ...
    print()
    print_to_file(board, filename, no_plys)
    if board.is_game_over():
        print("GAME OVER")


def print_to_file(board, filename, no_plys):
    f = open("game.txt", "a+")
...
def run(board, is_white, filename, no_plys=0):
    if is_white:
    ...
    no_plys += 1
    print_board(board, filename, no_plys)
    if board.is_game_over():
        print("GAME OVER")
        return
    else:
        run(board, not is_white, filename, no_plys)
